...
void getChatMembers(sqlite3 *conn, FullUserInfo memberList[30], int *memberCount, int chatID) {
    int result;
    char query[QUERY_SIZE] = {0};
    sqlite3_stmt *stmt;
...
void sqlAuthorize(sqlite3 *conn, FullUserInfo *user, AuthorizationPackage *auPackage) {
    ...
    sqlite3_prepare_v2(conn, query, (int) strlen(query), &stmt, NULL);
    auPackage->requestsCount = 0;
    while(sqlite3_step(stmt)==SQLITE_ROW){
        auPackage->requests[auPackage->requestsCount].ID = sqlite3_column_int(stmt, 0);
        strcpy(auPackage->requests[auPackage->requestsCount].username, (const char*) sqlite3_column_text(stmt, 1));
        strcpy(auPackage->requests[auPackage->requestsCount].password, "***");
        strcpy(auPackage->requests[auPackage->requestsCount].firstName, (const char*) sqlite3_column_text(stmt, 2));
        strcpy(auPackage->requests[auPackage->requestsCount].secondName, (const char*) sqlite3_column_text(stmt, 3));
        auPackage->requestsCount++;
    }
    sqlite3_finalize(stmt);
    memset(query, 0, QUERY_SIZE);
}
