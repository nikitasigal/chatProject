...
int main() {
    ...
    ZeroMemory(&hints,
               sizeof(hints));  // Обнуление области памяти, в которой находится структура. Потому что дальше могут возникнуть ошибки
    ...
    int getAddressInfoError = getaddrinfo(DEFAULT_SERVER_IP, DEFAULT_PORT, &hints,
                                          &result);    // Преобразовать текстовую информацию о хосте в адресную
    ...
    serverSocket = socket(result->ai_family, result->ai_socktype,
                          result->ai_protocol); // Создаём сокет, используя полученные данные о сети
    ...
    int connectError = connect(serverSocket, result->ai_addr,
                               (int) result->ai_addrlen); // Пытаемся подключиться к серверу
    if (connectError == SOCKET_ERROR) {
        printf("Socket connection error. Shutting down...");
        closesocket(
                serverSocket);  // Вообще, в документации написано, что надо проверить следующий IP, полученный в getaddrinfo, но поф
    ...
    int recBytes = recv(serverSocket, (void *) &recData, MESSAGE_LENGTH, 0);
